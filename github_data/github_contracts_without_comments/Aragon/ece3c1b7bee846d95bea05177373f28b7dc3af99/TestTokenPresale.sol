...
contract testtokenpresale {
  ...
  function throwswhennetworkplaceholderisbad() {
    ...
    ant a = new ant(new minimetokenfactory());
    a.changecontroller(sale);
    sale.setant(a, new anplaceholder(address(sale), address(sale)), new salewallet(sale.aragondevmultisig(), sale.finalblock(), address(sale))); 
  }
  ...
  function testsetpresaletokens() {
    ...
    assert.equal(minimeirrevocablevestedtoken(sale.token()).transferabletokens(0x2, uint64(now)), 0, );
    assert.equal(minimeirrevocablevestedtoken(sale.token()).transferabletokens(0x2, uint64(now + 8 weeks)), 2 finney, );
    assert.equal(minimeirrevocablevestedtoken(sale.token()).transferabletokens(0x2, uint64(now + 12 weeks)), 18 finney, );
    assert.equal(minimeirrevocablevestedtoken(sale.token()).transferabletokens(0x2, uint64(now + 24 weeks)), 36 finney, );
    assert.equal(minimeirrevocablevestedtoken(sale.token()).transferabletokens(0x1, uint64(now + 24 weeks)), 100 finney, );

    assert.equal(erc20(sale.token()).totalsupply(), 156 finney, );
