...
contract feepool is proxyable, selfdestructible, limitedsetup, mixinresolver {
    ...
    function feepoolstate() internal view returns (feepoolstate) {
        require(resolver.getaddress() != address(0), );
        return feepoolstate(resolver.getaddress());
    }

    function feepooleternalstorage() internal view returns (feepooleternalstorage) {
        require(
            resolver.getaddress() != address(0),
            
        );
        return feepooleternalstorage(resolver.getaddress());
    }

    function exchanger() internal view returns (iexchanger) {
        require(resolver.getaddress() != address(0), );
        return iexchanger(resolver.getaddress());
    }

    function issuer() internal view returns (iissuer) {
        require(resolver.getaddress() != address(0), );
        return iissuer(resolver.getaddress());
    }

    function synthetixstate() internal view returns (isynthetixstate) {
        require(resolver.getaddress() != address(0), );
        return isynthetixstate(resolver.getaddress());
    }

    function rewardescrow() internal view returns (isynthetixescrow) {
        require(resolver.getaddress() != address(0), );
        return isynthetixescrow(resolver.getaddress());
    }

    function delegateapprovals() internal view returns (delegateapprovals) {
        require(resolver.getaddress() != address(0), );
        return delegateapprovals(resolver.getaddress());
    }
    ...
    function convertxdrfeestosusd(address exchangeratesaddress) public optionalproxy_onlyowner {
        ...
        uint susdamount = iexchangerates(_exchangerates).effectivevalue(, xdramount, susd);
