...
contract issuer is owned, mixinresolver, iissuer {
    ...
    function liquidatedelinquentaccount(
        ...
        require(ierc20(address(synthetix().synths(susd))).balanceof(liquidator) >= susdamount, );
        ...
        uint collateralvalue = exchangerates().effectivevalue(, collateral, susd);
        ...
        if (totalredeemed > collateral) {
            ...
            amounttoliquidate = exchangerates().effectivevalue(, collateral.dividedecimal(safedecimalmath.unit().add(liquidationpenalty)), susd);
        }
