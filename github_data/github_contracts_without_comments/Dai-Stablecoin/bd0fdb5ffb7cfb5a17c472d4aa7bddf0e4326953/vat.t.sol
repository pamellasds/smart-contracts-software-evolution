...
contract bitetest is dstest {
    ...
    function test_bite_max_lump() public {
        ...
        vat.file(, rad(300000 ether));
        vat.file(, , rad(300000 ether));
        vat.file(, , ray(205 ether));
        vat.frob(, me, me, me, 1000 ether, 200000 ether);

        vat.file(, , ray(2 ether));  

        uint256 auction = cat.bite(, address(this));
    ...
    function testfail_bite_forced_over_max_lump() public {
        ...
        hevm.store(
            address(cat),
            bytes32(uint256(keccak256(abi.encode(bytes32(), uint256(1)))) + 2),
            bytes32(max_lump + 1)
        );
        cat.file(, uint256(1));
