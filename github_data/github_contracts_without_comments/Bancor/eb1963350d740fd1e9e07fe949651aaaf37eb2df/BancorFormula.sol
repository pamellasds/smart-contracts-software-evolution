...
contract bancorformula is ibancorformula, safemath {
    ...
    uint256 constant ceiling_ln2 = 0xb17217f8; 
    ...
    function lnupperbound(uint256 _numerator, uint256 _denominator) constant returns (uint256) {
        ...
        if (scalednumerator <= _denominator *  0x2b7e15162) 
            return uint256_1 << min_precision;
        if (scalednumerator <= _denominator *  0x763992e35) 
            return uint256_2 << min_precision;
        if (scalednumerator <= _denominator * 0x1415e5bf6f) 
